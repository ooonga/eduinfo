$.ajax({
  ...
  success: function(data){
    var dataTable = $('#get_json');
    $.each(data, function(index, data){
      var addData = '';
      addData += '<tr>';
      addData += '<td>'+data.number+'</td>';
      addData += '<td>'+data.title+'</td>';
      addData += '<td>'+data.years+'</td>';
      addData += '<td>'+data.date+'</td>';
      addData += '</tr>';
      dataTable.append(addData);
    })
    pageBoard();
  }
});
...
function pageBoard(){
  $('table.board').each(function() {
    ...
    var currentPage = 0;
    var listCount = 10;  //목록의 수
    var $table = $(this); 

    var listRows = $table.find('tbody tr').length;
    ...
    $table.bind('rePaginate', function() {
      $table.find('tbody tr').hide().slice(currentPage * listCount, (currentPage + 1) * listCount).show();
